      ...
      <div class="side-nav">
        <a-affix :offset-top="80">
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            ...
            <a-menu-item key="memory" v-if="memoryApplied && memoryProfile">
              <span>🧠 偏好记忆</span>
            </a-menu-item>
      ...
      <div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <a-card
              id="memory"
              v-if="memoryApplied && memoryProfile"
              title="🧠 已参考历史偏好"
              :bordered="false"
              class="memory-card"
            >
              <div class="memory-summary-row">
                <span class="memory-count">{{ memoryProfile.trip_count }}</span>
                <span class="memory-count-label">次成功规划记忆</span>
              </div>
              <div class="memory-chip-group">
                <a-tag v-if="memoryProfile.transportation" color="blue">
                  {{ memoryProfile.transportation }}
                </a-tag>
                <a-tag v-if="memoryProfile.accommodation" color="purple">
                  {{ memoryProfile.accommodation }}
                </a-tag>
                <a-tag
                  v-for="city in memoryProfile.recent_cities"
                  :key="`city-${city}`"
                  color="cyan"
                >
                  {{ city }}
                ...
                <a-tag
                  v-for="preference in memoryProfile.preferences"
                  :key="`pref-${preference}`"
                  color="green"
                >
                  {{ preference }}
...
import type { MemoryProfile, TripPlan } from '@/types'

const router = useRouter()
const tripPlan = ref<TripPlan | null>(null)
const memoryApplied = ref(false)
...
onMounted(async () => {
  ...
  if (data) {
    ...
    memoryApplied.value = sessionStorage.getItem('tripMemoryApplied') === 'true'
    const memoryProfileData = sessionStorage.getItem('tripMemoryProfile')
    memoryProfile.value = memoryProfileData ? JSON.parse(memoryProfileData) : null
    // 加载景点图片
